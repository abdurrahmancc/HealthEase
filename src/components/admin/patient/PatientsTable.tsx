...
const PatientsTable = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`${process.env.NEXT_PUBLIC_baseURL}/v1/api/Users/GetUsers?pageNumber=${pageFromUrl}&pageSize=${sizeFromUrl}&search=${searchFromUrl}&role=Patient`, {
    ...
    const updateUrl = (page: number, size: number, searchText: string) => {
        ...
        const newUrl = `/admin/patients?${params.toString()}`;
                                ...
                                <table className="table w-full">
                                    ...
                                    <tbody className="cursor-pointer">
                                        {users?.length > 0 && users.map((user: User, index: number) => (
                                            <PatientsTableRow key={user.id} user={user} index={index} setDeleteModal={setDeleteModal} setInputRoleId={setInputRoleId} />
                                        ))}
                                    </tbody>
                                </table>
                                {
